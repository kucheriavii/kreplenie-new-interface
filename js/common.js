...
var scroolBar = function(){
    ...
    $(".catalog__item-gallery").mCustomScrollbar({
        axis:"y",
        scrollInertia: 0,
        scrollButtons: {enable: true}
    });
...
var showMore = function(){
    //скрывать/отображать лишние елементы (про том менять надпись на кнопке)
    var text,
...
var horizontalScrollItem = function (e) {
    ...
    //ПЕРЕРОБЛЯЮ ЦЮ ФУНКЦЫЮ
    $('.catalog__item-gallery').each(function(){

        if($(this).find('.catalog__item-gallery-item').length>5){

            $(this).css({'margin-top':'30px', 'padding-bottom':'80px'});
            $("<span class='accessories-items-left-scroll'></span>").insertBefore($(this));
            $("<span class='accessories-items-right-scroll'></span>").insertAfter($(this));
...
var swichVersion = function(){
  //перезагружать страницу при достижении 1280px
...
var popup = function(e){
    //Закрывать попап по нажатию на крестик закрытия
    ...
    //попап регистрации - срабатывает по нажатию на кнопке логин/регестрация
    $('.main-menu__item--right').click(function (e) {
        e.preventDefault();
    ...
    /*Закрывать попап по нажатию вне зоны попапа*/
